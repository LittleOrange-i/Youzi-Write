                          ...
                          <template #reference>
                            ...
                            >
                              ...
                              <div
                                ...
                                @mousedown.stop="startResize($event, event, 'left')"
                                title="拖拽调整起始时间"
                              ></div>
                              ...
                              <div
                                ...
                                @mousedown.stop="startResize($event, event, 'right')"
                                title="拖拽调整结束时间"
                              ></div>
                            </div>
                          </template>
...
// 调整大小相关数据
const resizingEvent = ref(null) // 当前正在调整大小的事件
const resizeType = ref(null) // 'left' | 'right' | null，表示从左侧还是右侧调整
const resizeStartX = ref(0) // 鼠标起始位置
const resizeStartTime = ref(0) // 原始的开始时间或结束时间
...
// 开始调整大小（左侧或右侧）
const startResize = (event, eventData, type) => {
  event.preventDefault()
  event.stopPropagation() // 阻止冒泡，避免触发拖拽

  resizingEvent.value = eventData
  resizeType.value = type
  resizeStartX.value = event.clientX
  resizeStartTime.value = type === 'left' ? eventData.startTime : eventData.endTime
  ...
  // 设置样式
  document.body.style.cursor = type === 'left' ? 'ew-resize' : 'ew-resize'
...
const handleResize = (event) => {
  ...
  // 计算单元格数量的变化（每个单元格40px）
  const cellChange = Math.round(deltaX / 40)

  // 查找当前事件所属的图表
  const chart = sequenceCharts.value.find((c) => c.events.includes(resizingEvent.value))
  if (!chart) return

  if (resizeType.value === 'left') {
    // 调整左侧（起始时间）
    const newStartTime = Math.max(1, resizeStartTime.value + cellChange)
    // 确保起始时间不超过结束时间
    if (newStartTime < resizingEvent.value.endTime) {
      resizingEvent.value.startTime = newStartTime
    }
  } else if (resizeType.value === 'right') {
    // 调整右侧（结束时间）
    const newEndTime = Math.min(chart.cellCount, resizeStartTime.value + cellChange)
    // 确保结束时间不小于起始时间
    if (newEndTime > resizingEvent.value.startTime) {
      resizingEvent.value.endTime = newEndTime
    }
  }
}
...
const stopResize = async () => {
  ...
  const chart = sequenceCharts.value.find((c) => c.events.includes(resizingEvent.value))
  if (chart && hasMovedWhileMouseDown.value) {
    ElMessage.success(
      `事件"${resizingEvent.value.introduction}"时间已调整为 ${resizingEvent.value.startTime}-${resizingEvent.value.endTime}`
    )
    await saveSequenceCharts()

    // 调整结束后，短暂抑制 mouseup 触发编辑
