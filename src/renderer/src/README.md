...
- `src/components`: 公用 UI 组件 <!-- 存放所有通用的 UI 组件 -->
  ...
  - `BookShelf`: 书架管理组件 <!-- 书籍展示与管理相关组件 -->
    - **分类体系升级**：作品分类采用级联选择器（Cascader），支持男频、女频及通用三大分类体系，提供更清晰的作品归类。 <!-- 优化书籍分类逻辑与交互体验 -->
...
- **编辑器面板 ([EditorPanel.vue](file:///c:/Users/FN\Desktop/youziwrite/src/renderer/src/components/Editor/EditorPanel.vue))**: 
  ...
  - **人物档案增强**: 
  - 默认使用 #e198b8 作为标记色，支持快速识别 <!-- 优化角色管理视觉效果 -->
  - **预览体验优化**：预览弹窗现在显示人物的全部详细信息，并支持通过 **Esc 键** 或 **点击遮罩层** 快速关闭。 <!-- 提升预览操作的便捷性 -->
  - **预览交互增强**：预览弹窗底部固定“编辑”与“确定”按钮，支持从预览直接跳转至编辑界面，实现闭环操作。 <!-- 优化人物管理流程 -->
  - **安全性保证**：编辑和新建弹窗依然保持“锁定”状态（禁止点击遮罩层或 Esc 关闭），防止误操作导致数据丢失。 <!-- 确保数据编辑安全 -->
  - **批量导入功能**：支持从 .xlsx 文件批量导入人物数据，包含头像、姓名、性别、别名、年龄、身高及详细介绍 <!-- 实现高效的角色初始化 -->
  - 数据校验机制：导入时自动校验数据格式（如性别选项、数字类型、字数限制等），并提供详细的错误定位提示 <!-- 确保数据质量 -->
- **工具栏功能 ([EditorToolbar.vue](file:///c:/Users/FN\Desktop/youziwrite/src/renderer/src/components/Editor/EditorToolbar.vue))**: <!-- 工具栏功能模块 -->
  - **快捷键动态提示**：在每个功能按钮下方显示对应的快捷键组合（如 Alt+Q），并支持随用户自定义配置实时更新。 <!-- 实现动态快捷键可视化提示 -->
  - **独立窗口模式**：支持将地图、时间线、人物谱等七大核心工具以独立窗口形式打开，实现多窗口协同作业。 <!-- 提供灵活的窗口组织方式 -->
  - 集成随机起名、世界地图、时间线、词条字典、人物谱、关系图、事序图、组织架构、禁词管理等核心创作辅助工具。 <!-- 提供一站式创作辅助入口 -->
  - **字数设置优化**：支持为不同书籍设置独立的段落字数上限，提升排版灵活性。 <!-- 实现书籍维度的个性化配置 -->
